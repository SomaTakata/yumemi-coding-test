...
jest.mock(
  "@/feature/populationTrends/components/RegionToggleButton",
  () =>
    ({ allSelected, region, toggleRegion }: any) => (
      <button onClick={() => toggleRegion(region)}>
        {allSelected ? "全て選択解除" : "全て選択"}
      </button>
    ),
);

jest.mock(
  "@/feature/populationTrends/components/PrefectureButton",
  () =>
    ({ name, onClick, selected }: any) => (
